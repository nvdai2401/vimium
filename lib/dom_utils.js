const DomUtils = {
  ...
  // Inject theme CSS variables. Called before injectUserCss so user CSS takes precedence.
  injectThemeCss(themeName) {
    const name = themeName || Settings.get("theme");
    const css = Themes.generateCss(name);
    const existing = document.querySelector("#vimium-theme-css");
    if (existing) existing.remove();
    if (!css) return;
    const style = document.createElement("style");
    style.type = "text/css";
    style.id = "vimium-theme-css";
